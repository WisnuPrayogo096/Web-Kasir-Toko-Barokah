...
function insert_produk()
{
    ...
    // Check if barcode array is set and not empty
    if (isset($_POST['barcode']) && !empty($_POST['barcode'])) {
        $existing_barcodes = array(); // Array to store existing barcodes

        // Process each barcode in the array
        foreach ($_POST['barcode'] as $barcodeString) {
            // Explode the string of barcodes into an array
            $barcodes = explode(',', $barcodeString);

            // Check if any barcode already exists in the barcode table
            foreach ($barcodes as $singleBarcode) {
                $no_barcode = trim($singleBarcode); // Remove leading/trailing whitespaces

                $result_existing_barcode = mysqli_query($koneksi, "SELECT COUNT(*) as count FROM barcode WHERE no_barcode='$no_barcode'");
                $row_existing_barcode = mysqli_fetch_assoc($result_existing_barcode);
                $count = $row_existing_barcode['count'];

                if ($count > 0) {
                    // Barcode already exists, show an alert or handle as needed
                    echo "<script>alert('Barcode $no_barcode sudah terdaftar.');</script>";
                    return false; // Stop the process if barcode already exists
                }

                // Add the barcode to the existing_barcodes array
                $existing_barcodes[] = $no_barcode;
            }
        }
    }

    // Insert product into tb_produk
    mysqli_query($koneksi, "INSERT INTO tb_produk SET kdproduk='$kdproduk', nm_produk='$nm_produk', kategori='$kategori', stok='$stok', harga='$harga'");

    // Get the ID of the inserted product
    $id_produk = mysqli_insert_id($koneksi);

    // Process each barcode in the array
    foreach ($existing_barcodes as $no_barcode) {
        // Insert each barcode into the barcode table
        mysqli_query($koneksi, "INSERT INTO barcode SET id_barang='$id_produk', no_barcode='$no_barcode'");
    }
...
function update_produk()
{
	...
	return mysqli_query($koneksi, "UPDATE tb_produk SET kdproduk='$kdproduk', nm_produk='$nm_produk', kategori='$kategori', stok='$stok', rak='', supplier='', harga='$harga' WHERE id='$id'");
}
...
function jumlah_saldo()
{
	...
	return rupiah($row['jtotal']);
}
function jumlah_terjual()
{
	global $koneksi;
	$jumlah = mysqli_query($koneksi, "SELECT IFNULL(SUM(jumlah_beli), 0) as jbeli from laporan_penjualan");
	$row = mysqli_fetch_array($jumlah);
	return $row['jbeli'];
}
